...
<?if ($arResult["isFormNote"] != "Y") {
	...
	foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
		?>
        <? if ($FIELD_SID == 'specialist'): ?>
            <?= str_replace('value=""','value="'.($arParams['SPECIALIST_ID']).'"',$arQuestion["HTML_CODE"]) ?>
            <? continue; ?>
        <? endif; ?>
		<div class="col-margin-bottom">
            ...
            <?= str_replace(array('<input','<select','<textarea'),array('<input class="input col-6"'.$req,'<select class="styler col-3"','<textarea class="input col-6"'.$req),$arQuestion["HTML_CODE"]) ?>
		</div>
	<?
	} //endwhile
	?>
	...
	if ($arResult["isUseCaptcha"] == "Y") {
		...
		<div class="col-margin-bottom">
			...
			<input class="input" type="text" name="captcha_word" size="30" maxlength="50" value="" class="inputtext" required />
		</div>
	<?
	} // isUseCaptcha
	?>
