...
$arResult['SERVICE_CLINICS'] = array();
$arDep = array();
$arResult['SERVICE_EMPLOYES'] = MCSchedule::GetEmployesForService($arResult['ID'],true);
$arUserIDs = array_keys($arResult['SERVICE_EMPLOYES']);
$arTime = MCSchedule::GetScheduleInfo($arUserIDs);
foreach ($arResult['SERVICE_EMPLOYES'] as $eID=>$eValue) {
    $arResult['SERVICE_EMPLOYES'][$eID]['WORKING_TIME'] = $arTime[$eID];
	if (!array_key_exists('UF_SPECIALITY',$eValue) || empty($eValue['UF_SPECIALITY']))
		unset ($arResult['SERVICE_EMPLOYES'][$eID]);
	else {
		foreach ($eValue['UF_DEPARTMENT'] as $depID) {
			$arOrg = array();
			if (!array_key_exists($depID,$arDep)) {
				$rsDep = CIBlockSection::GetList(array(),array('IBLOCK_ID'=>$arParams['DEP_IB_ID'],'ID'=>$depID),false, array('ID','UF_*'));
				$arDepartment = $rsDep->GetNext();
				$arDep[$depID] = $arDepartment;
				if (array_key_exists('UF_ORGANIZATION',$arDep[$depID])) {
					foreach ($arDep[$depID]['UF_ORGANIZATION'] as $orgID) {
						if (array_key_exists($orgID,$arResult['SERVICE_CLINICS'])) continue;
						$rsOrg = CIBlockElement::GetList(array('NAME'),array('ID'=>$orgID));
						$arOrg = $rsOrg->GetNextElement();
						$arResult['SERVICE_CLINICS'][$orgID] = $arOrg->fields;
						$arResult['SERVICE_CLINICS'][$orgID]['PROPERTIES'] = $arOrg->GetProperties();
					}
				}
			}
		}
...
if (!IsModuleInstalled('sale')) {
	$rsPrices = CIBlockElement::GetList(array(),array('IBLOCK_ID'=>$arParams['PRICE_IB_ID'],'PROPERTY_SERVICE'=>$arResult['ID']),false,false,array('ID','PROPERTY_SERVICE','PROPERTY_PRICE'));
	if($arPrice = $rsPrices->GetNext()) {
		$arResult['MIN_PRICE']['VALUE'] = $arPrice['PROPERTY_PRICE_VALUE'];
	}
	$arResult['MIN_PRICE']['CURRENCY'] = 'RUB';
}

if (is_array($arResult['PREVIEW_PICTURE'])) {
    $arResult['PREVIEW_PICTURE'] = CFile::ResizeImageGet($arResult['PREVIEW_PICTURE'], array('width'=>250, 'height'=>150), BX_RESIZE_IMAGE_EXACT, true);
} else {
    $arResult['PREVIEW_PICTURE'] = null;
}
if (is_array($arResult['DETAIL_PICTURE'])) {
    $arResult['DETAIL_PICTURE'] = CFile::ResizeImageGet($arResult['DETAIL_PICTURE'], array('width'=>1020, 'height'=>500), BX_RESIZE_IMAGE_EXACT, true);
} else {
    $arResult['DETAIL_PICTURE'] = null;
}

$arResult['CAN_RECORD'] = !empty($arResult['SERVICE_EMPLOYES']) && empty($arResult['PROPERTIES']['not_for_record']['VALUE']) ? count($arResult['SERVICE_EMPLOYES']) : 0;
$this->__component->SetResultCacheKeys(array('CAN_RECORD'));
if (is_array($arResult['MIN_PRICE'])) {
