                        ...
                        <td>
                            ...
                            <div class="reviews-post-reply-buttons">
                                ...
                                if ($arResult["FORUM"]["ALLOW_QUOTE"] == "Y"):
                                    ?>
                                    <a href="#review_anchor" title="<?= GetMessage("F_QUOTE_HINT") ?>"
                                ...
                                ?>
                                <a href="#review_anchor" title="<?= GetMessage("F_NAME") ?>"
                                   class="reviews-button-small" <?
    ...
    <div class="reviews-reply-fields">
        <div class="reviews-reply-field-user">
            <div class="reviews-reply-field reviews-reply-field-author"><label
                    for="REVIEW_AUTHOR<?= $arParams["form_index"] ?>"><?=
                    GetMessage("OPINIONS_NAME")?><?
                    ?><span class="reviews-required-field">*</span></label>
                <span><input name="REVIEW_AUTHOR" id="REVIEW_AUTHOR<?= $arParams["form_index"] ?>" size="30" type="text"
                             value="<?= $arResult["REVIEW_AUTHOR"] ?>" tabindex="<?= $tabIndex++; ?>"/></span></div>
            <?
            if ($arResult["FORUM"]["ASK_GUEST_EMAIL"] == "Y"):
                ?>
                <div class="reviews-reply-field-user-sep">&nbsp;</div>
                <div class="reviews-reply-field reviews-reply-field-email"><label
                        for="REVIEW_EMAIL<?= $arParams["form_index"] ?>"><?= GetMessage("OPINIONS_EMAIL") ?><span
                            class="reviews-required-field">*</span></label>
                    <span><input type="text" name="REVIEW_EMAIL" id="REVIEW_EMAIL<?= $arParams["form_index"] ?>"
                                 size="30" value="<?= $arResult["REVIEW_EMAIL"] ?>"
...
<div class="reviews-bbcode-line" id="forum_bbcode_line<?= $arParams["form_index"] ?>">
    ...
    if ($arResult["FORUM"]["ALLOW_BIU"] == "Y"):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-bold" id="form_b"
           title="<?= GetMessage("F_BOLD") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-italic" id="form_i"
           title="<?= GetMessage("F_ITAL") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-underline" id="form_u"
           title="<?= GetMessage("F_UNDER") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-strike" id="form_s"
           title="<?= GetMessage("F_STRIKE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    if ($arResult["FORUM"]["ALLOW_QUOTE"] == "Y"):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-quote" id="form_quote"
           title="<?= GetMessage("F_QUOTE_TITLE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    if ($arResult["FORUM"]["ALLOW_CODE"] == "Y"):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-code" id="form_code"
           title="<?= GetMessage("F_CODE_TITLE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    if ($arResult["FORUM"]["ALLOW_ANCHOR"] == "Y"):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-url" id="form_url"
           title="<?= GetMessage("F_HYPERLINK_TITLE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    if ($arResult["FORUM"]["ALLOW_IMG"] == "Y"):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-img" id="form_img"
           title="<?= GetMessage("F_IMAGE_TITLE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    if ($arResult["FORUM"]["ALLOW_VIDEO"] == "Y"):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-video" id="form_video"
           title="<?= GetMessage("F_VIDEO_TITLE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    ...
    if ($arResult["FORUM"]["ALLOW_LIST"] == "Y"):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-list" id="form_list"
           title="<?= GetMessage("F_LIST_TITLE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    if ($arResult["FORUM"]["ALLOW_FONT"] == "Y"):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-color" id="form_palette"
           title="<?= GetMessage("F_COLOR_TITLE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    if (LANGUAGE_ID == 'ru'):
        ?>
        <a href="#postform" class="reviews-bbcode-button reviews-bbcode-translit" id="form_translit"
           title="<?= GetMessage("F_TRANSLIT_TITLE") ?>">
            <img src="/bitrix/components/bitrix/forum.post_form/templates/.default/images/bbcode/empty_for_ie.gif"/></a>
    <?
    endif;
    if ($arResult["FORUM"]["ALLOW_FONT"] == "Y"):
...
if ($arResult["FORUM"]["ALLOW_SMILES"] == "Y"):
    ...
    if ($arParams["SMILES_COUNT"] <= 0):
        ...
        <div class="reviews-smiles-line reviews-smiles-corrected reviews-smiles-none"
             id="forum_smiles_line<?= $arParams["form_index"] ?>">
            <?
            foreach ($arResult["SMILES"] as $res):
                ...
                <span class="reviews-smiles-item" style="height:<?= $iMaxH ?>px;"><?
                    ?><a href="#postform" name="smiles"
                         style="margin-top:<?= (round(($iMaxH - $res['IMAGE_HEIGHT']) / 2)) ?>px;"><?
    ...
    elseif ($arParams["SMILES_COUNT"] < count($arResult["SMILES"])):
        ?>
        <div class="reviews-clear-float"></div>
    <div class="reviews-smiles-line" id="forum_smiles_showed"
         style="display:<?= ($arUserSettings["smiles"] == "show" ? "none" : "block") ?>;">
        <?
        ...
        ?>
        <div class="reviews-smiles-line" id="forum_smiles_hidden"
             style="display:<?= ($arUserSettings["smiles"] == "show" ? "block" : "none") ?>;">
            <?
            foreach ($arResult["SMILES"] as $res):
                $TYPING = strtok($res['TYPING'], " ");
                ?>
...
<div class="reviews-reply-field reviews-reply-field-text">
    <textarea class="post_message" cols="55" rows="14" name="REVIEW_TEXT" id="REVIEW_TEXT"
              tabindex="<?= $tabIndex++; ?>"><?= $arResult["REVIEW_TEXT"]; ?></textarea>
</div>
        ...
        if (!empty($arResult["REVIEW_FILES"])):
            foreach ($arResult["REVIEW_FILES"] as $key => $val):
                ...
                <div class="reviews-uploaded-file">
                    ...
                    <input type="checkbox" name="FILES_TO_UPLOAD[<?= $key ?>]" id="FILES_TO_UPLOAD_<?= $key ?>"
                           value="<?= $key ?>" checked="checked"/>
                    <label for="FILES_TO_UPLOAD_<?= $key ?>"><?= $val["ORIGINAL_NAME"] ?> (<?= $val["CONTENT_TYPE"] ?>
                        ) <?= $sFileSize ?>
                        ( <a
                            href="/bitrix/components/bitrix/forum.interface/show_file.php?action=download&amp;fid=<?= $key ?>"><?= GetMessage("F_DOWNLOAD") ?></a>
                        )
                    </label>
                </div>
            <?
...
<div class="reviews-reply-field reviews-reply-field-settings">
    ...
    if ($arResult["SHOW_SUBSCRIBE"] == "Y"):
        ?>
        <div class="reviews-reply-field-setting">
            <input type="checkbox" name="TOPIC_SUBSCRIBE" id="TOPIC_SUBSCRIBE<?= $arParams["form_index"] ?>"
                   value="Y" <?
            ?><?=
        ...
        if ($arResult["FORUM_SUBSCRIBE"] == "Y"):
            ?>
            <div class="reviews-reply-field-setting">
                <input type="checkbox" name="FORUM_SUBSCRIBE" id="FORUM_SUBSCRIBE<?= $arParams["form_index"] ?>"
                       value="Y" <?
                ?><?=
                ($arResult["FORUM_SUBSCRIBE"] == "Y") ? "checked disabled " : "";?> tabindex="<?= $tabIndex++; ?>"/><?
                ?>&nbsp;<label
                    for="FORUM_SUBSCRIBE<?= $arParams["form_index"] ?>"><?= GetMessage("F_WANT_SUBSCRIBE_FORUM") ?></label>
            </div>
        <?
        endif;
    endif;
