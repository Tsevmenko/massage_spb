...
function CreatePalette() {
	...
	for (var ii = 0; ii < 216; ii++) {
		...
		rgb['R'] = ii % 6;
		rgb['G'] = Math.round(ii / 36) % 6;
		rgb['B'] = Math.round(ii / 6) % 6;
...
function PostForm() {
	...
	this.Init = function (form) {
		...
		if (oDivs && oDivs.length > 0) {
			for (var ii = 0; ii < oDivs.length; ii++) {
				...
				oDiv.onmouseout = function () {
					this.className = this.className.replace(/marked/, '').replace('  ', ' ');
				};
		...
		if (this.form['FONT']) {
			this.form['FONT'].onmousedown = function (e) {
				t.saveSelection = t.GetSelection();
			};
			this.form['FONT'].onchange = function (e) {
				t.format_text(this, e)
			};
			this.form['FONT'].onmouseover = function () {
				this.className += ' marked';
			};
			this.form['FONT'].onmouseout = function () {
				this.className = this.className.replace(/marked/, '').replace('  ', ' ');
			};
		}
		// Smiles
		...
		this.form["REVIEW_TEXT"].onfocus = function (e) {
			this.hasfocus = true
		};
		this.form["REVIEW_TEXT"].onblur = function (e) {
			this.hasfocus = false;
		};
		...
		this.format_text = function (oObj, e) {
			...
			if (this.tags[id] == 'simple_tag') {
				...
				if ((!jsUtils.in_array(tag_name, this.stack) || this.GetSelection()) && !(tag_name == 'FONT' && oObj.value == 'none')) {
		...
		this.show_smiles_dinamic = function (oObj, status, send_data) {
			...
			if (!this.oDivSmiles) {
				...
				res.onclick = function () {
					t.show_smiles_dinamic(this, 'hide');
					return false;
				}
