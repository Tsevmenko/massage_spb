...
if (!is_array($arResult['USERS']) || !($USERS_CNT = count($arResult['USERS']))):
    ...
    foreach ($arResult['DEPARTMENTS'] as $arDept) {
        $arDeptsChain[$arDept['DEPTH_LEVEL']] = '<a href="'.$arParams['STRUCTURE_PAGE'].'?set_filter_'.$arParams['STRUCTURE_FILTER'].'=Y&'.$arParams['STRUCTURE_FILTER'].'_UF_DEPARTMENT='.$arDept['ID'].'">'.$arDept['NAME'].'</a>';
        if (count($arDept['USERS']) > 0) {
            echo '<div class="users-departments-chain">'.implode('&nbsp;|&nbsp;', array_slice($arDeptsChain, 0, $arDept['DEPTH_LEVEL'])).'</div>';
            ?>
            <div class="bx-users">
                <?
