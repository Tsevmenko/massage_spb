...
if (!is_array($arResult['USERS']) || !($USERS_CNT = count($arResult['USERS']))):
    ...
    <table class="bx-users-table data-table" style="border: 0px" width="100%">
        ...
        foreach ($arResult['DEPARTMENTS'] as $arDept) {
            $arDeptsChain[$arDept['DEPTH_LEVEL']] = '<a style="font-size:15px; color:#000;" href="'.$arParams['STRUCTURE_PAGE'].'?set_filter_'.$arParams['STRUCTURE_FILTER'].'=Y&'.$arParams['STRUCTURE_FILTER'].'_UF_DEPARTMENT='.$arDept['ID'].'">'.$arDept['NAME'].'</a>';
            ...
            <tr>
                <td colspan="<?= count($arParams['USER_PROPERTY']) ?>">
                    ...
                    <div class="users-departments-chain"
                         style="margin-bottom:4px;"><?= implode('&nbsp;-&nbsp;', array_slice($arDeptsChain, 0, $arDept['DEPTH_LEVEL'])) ?></div>
                </td>
            </tr>
