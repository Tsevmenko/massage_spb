...
<? if (is_array($arResult["PROPERTY_LIST"]) && count($arResult["PROPERTY_LIST"]) > 0): ?>
	...
	<div class="mb10"><? if (intval($propertyID) > 0): ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else: ?><?= !empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID) ?><?endif ?><? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])): ?>
        ...
        switch ($INPUT_TYPE):
            ...
            case "HTML":
                ...
                $LHE->Show(array(
                        ...
                        <? foreach ($arResult["PATTERNS"] as $keyP => $pattern): ?>
                            <div id="pattern_code_<?= $keyP ?>" style="display:none;">
                                <?= $pattern['DETAIL_TEXT'] ?>
                            </div>
                        <? endforeach ?>
				</div>
            ...
            case "N":
                for ($i = 0; $i < $inputNum; $i++) {
                    ...
                    ?>
                    <input class="input col-6" type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="25" <? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])): ?>required<?endif?>
                           value="<?= $value ?>" /><?
            ...
            case "L":
                ...
                switch ($type):
                    ...
                    case "multiselect":
                        ?>
                        <select  class="styler col-6"
                            name="PROPERTY[<?= $propertyID ?>]<?= $type == "multiselect" ? "[]\" size=\"".$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]."\" multiple=\"multiple" : "" ?>" <? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])): ?>required<?endif?>>
                            <?
                            if (intval($propertyID) > 0)
    ...
    <? if ($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0): ?>
		...
		<div class="col-margin-bottom">
			<div class="mb10"><?= GetMessage("IBLOCK_FORM_CAPTCHA_PROMPT") ?><span class="starrequired">*</span>:</div>
            <input class="input col-4" type="text" name="captcha_word" maxlength="50" value="">
   		 </div>
    <? endif ?>
<? endif ?>
<div class="col-margin-bottom">
	<input class="btn" type="submit" name="iblock_submit" value="<?= GetMessage("IBLOCK_FORM_SUBMIT") ?>"/>
        &nbsp;&nbsp;
        <input class="btn" type="submit" name="delete" value="<?= GetMessage("IBLOCK_FORM_DELETE") ?>"
               onClick="return confirm('<?= GetMessage("FORM_DELETE_CONFIRMATION") ?>');"/>
