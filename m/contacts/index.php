...
<div class="negative-margin gray">
	<div class="steps">
		<?if (isset($_REQUEST['cat'])):?>
			<a href="<?=$APPLICATION->GetCurPageParam('',array('CAT','item'))?>" class="goback ir"><?=GetMessage('MCRWizardStep_BACK')?></a>
		<?endif;?>
		Офисы клиники
	</div> <!-- .steps -->
	<section>
...
<?if (IsModuleInstalled('bitrix.map')):?>
	...
	"CHECK_DATES" => "Y",
	"DETAIL_URL" => '/m/schedule/?STEP=where_to_record&COMPANY=#ID#',
	"CACHE_TYPE" => "A",
	"CACHE_TIME" => "36000000",
	"CACHE_FILTER" => "N",
	"CACHE_GROUPS" => "Y",
...
<?else:?>
   <?$APPLICATION->IncludeComponent("bitrix:news.list", "contact_list", array(
	...
	"CHECK_DATES" => "Y",
	"DETAIL_URL" => "/m/contacts/detail.php?ID=#ID#",
	"AJAX_MODE" => "N",
	"AJAX_OPTION_JUMP" => "N",
	"AJAX_OPTION_STYLE" => "Y",
